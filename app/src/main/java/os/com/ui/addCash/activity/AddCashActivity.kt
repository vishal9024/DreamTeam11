...
class AddCashActivity : BaseActivity(), View.OnClickListener, CFClientInterface /*PaymentResultListener*/ {
    ...
    private fun showPaymentOption(
        ...
        dialogue.ll_cashfree.setOnClickListener {
            //            payUsingRazorPay()
            payUsingCashfree()
            dialogue.dismiss()
        }
...
//                    generatePayTmCheckSumRequest.CHECKSUMHASH = response.response!!.data!!.checksum
                try {
                    if (response.status.equals("OK", ignoreCase = true)) {
                        ...
                        AppDelegate.showToast(this@AddCashActivity, response.message)
                    }
