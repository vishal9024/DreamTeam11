    ...
    override fun onClick(view: View?) {
        try {
                ...
                R.id.txt_ApplyCode -> {
                    if (!et_email.text.toString().isEmpty())
                        if (!et_email.text.toString().isEmpty()) {
                            ...
                            Toast.makeText(context!!, getString(R.string.please_enter_code), Toast.LENGTH_LONG).show()
            }
            R.id.txt_Withdraw -> {
                  try{
                      if (accountVerified) {
            ...
            R.id.imvJoinInfo -> {
                ...
                intent.putExtra(
                    "PAGE_SLUG",
                    activity!!.resources.getString(R.string.app_name) + " Champions"
                )
                ...
                if (response.response!!.status) {
                    ...
                    if (mData != null)
                        currentBalance =
                            (mData!!.getCash_bonus_amount().toFloat() + mData!!.total_cash_amount.toFloat() + mData!!.total_winning_amount.toFloat()).toString()
                    startActivity(
                        Intent(activity, AddCashActivity::class.java).putExtra(Tags.DATA, response.response!!.data)
